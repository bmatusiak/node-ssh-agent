...
SSHAgentClient.prototype.sign = function (key, data, callback) {
  ...
  function signRequest() {
    ...
    var request = Buffer.alloc(4 + 1 + 4 + key._raw.length + 4 + data.length + 4);
...
SSHAgentClient.prototype._request = function (getRequest,
  ...
  socket.on('data', function (data) {
    ...
    console.log("message", type, len);
    return parseResponse(data.slice(5, len));
  });
